...
wkFocus::session_tbl %>% 
  ...
  dplyr::mutate(sid = paste0("res", round, gid), mins = duration/60) %>% 
  dplyr::select(sid, round, group = gid, phenomenon, mins) %>% 
  knitr::kable(digits = 0,
    col.names = c("Session ID", "Round", "Group ID", "Phenomenon", "Length (min)"),
...
pander::pander(wkFocus::focus_code_description, 
...
pander::pander(wkFocus::facil_code_description, 
               justify = c("left", "left", "left"), 
               split.tables = 100, split.cells = c(2, 13, 70))
